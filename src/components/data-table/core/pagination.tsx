...
export function DataTablePagination<TData>({
  ...
  const pageItems = pageNumbers.map((pageNumber) => {
    ...
    const key = hasLeadingEllipsis
      ? 'pagination-ellipsis-trailing'
      : 'pagination-ellipsis-leading'
      ...
      <div className='flex min-w-0 shrink-0 items-center gap-2 @xl/pagination:gap-3'>
        ...
        <div className='flex min-w-0 shrink-0 items-center gap-1 @lg/pagination:gap-1.5 @xl/pagination:gap-2'>
          ...
          {pageItems.map((pageItem) => (
            <div key={pageItem.key} className='flex items-center'>
              {pageItem.pageNumber === '...' ? (
                ...
                <Button
                  variant={
                    currentPage === pageItem.pageNumber ? 'default' : 'outline'
                  }
                  className={cn(
                  ...
                  onClick={() =>
                    table.setPageIndex((pageItem.pageNumber as number) - 1)
                  }
                >
                  <span className='sr-only'>
                    {t('Go to page {{page}}', { page: pageItem.pageNumber })}
                  </span>
                  {pageItem.pageNumber}
                </Button>
              )}
