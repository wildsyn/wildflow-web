...
describe('WildFlow Job code samples', () => {
  test('renders a copyable public ASR request with its required artifact and idempotency key', () => {
    const sample = buildWildFlowJobSample('curl', {
      apiKeyEnv: 'NEW_API_KEY',
      modelName: 'wildflow/dual-asr-v1',
    })

    assert.match(sample, /https:\/\/api\.wildflow\.cn\/v1\/jobs/)
    assert.match(sample, /Idempotency-Key/)
    assert.match(sample, /wildflow\/dual-asr-v1/)
